...
# Retrieval knobs
RETRIEVE_TOP_K = int(os.getenv("RETRIEVE_TOP_K", "250"))     # candidates before hard filters
LLM_PACK_TOP_N = int(os.getenv("LLM_PACK_TOP_N", "30"))      # sent to DeepSeek chat
RETURN_TOP_N = int(os.getenv("RETURN_TOP_N", "10"))          # API returns top N

PROMPT_VERSION = os.getenv("PROMPT_VERSION", "v3-deepseek-embeddings")

# DeepSeek (chat)
DEEPSEEK_API_KEY = os.getenv("DEEPSEEK_API_KEY", "").strip()
DEEPSEEK_URL = os.getenv("DEEPSEEK_URL", "").strip()  # e.g. https://api.deepseek.com/v1/chat/completions
DEEPSEEK_MODEL = os.getenv("DEEPSEEK_MODEL", "deepseek-chat")
DEEPSEEK_TEMPERATURE = float(os.getenv("DEEPSEEK_TEMPERATURE", "0.2"))

# DeepSeek (embeddings) — OpenAI-compatible endpoint
# Base default is https://api.deepseek.com/v1/embeddings
DEEPSEEK_EMBED_URL = os.getenv("DEEPSEEK_EMBED_URL", "https://api.deepseek.com/v1/embeddings").strip()
# You MUST set an embeddings-capable model id here.
DEEPSEEK_EMBED_MODEL = os.getenv("DEEPSEEK_EMBED_MODEL", "").strip()  # e.g. "text-embedding-3-small" if supported by your provider
...
def init_db() -> None:
    ...
    # Hosted embeddings cache (program text -> embedding vector)
    cur.execute(
        """
...
# ============================================================
# UTIL: hashing / cache keys / cosine sim
# ============================================================
...
def cache_set(cache_key: str, response: Dict[str, Any]) -> None:
    ...
    cur.execute(
        """
        INSERT OR REPLACE INTO response_cache (cache_key, created_at, response_json)
        VALUES (?, ?, ?)
        """,
...
class StudentInput(BaseModel):
    ...
    # IMPORTANT: Pakistan commonly uses 4.0 as best (higher is better).
    # If you have a system where LOWER is better (e.g., 1.0 best), set "lower_is_better".
...
# ============================================================
# GPA conversion (fix: handle both grading directions)
# ============================================================
...
def convert_to_german_grade(
    ...
    """
    Convert from arbitrary grading scale to German grade estimate using a modified Bavarian-style formula.

    - If scale_direction == "higher_is_better" (e.g. Pakistan 1..4 where 4 is best):
        best = max_grade, worst passing = min_passing_grade
    - If scale_direction == "lower_is_better" (e.g. some systems where 1 is best):
        best = min_passing_grade (or min on scale), worst passing = max_grade
        We invert by mapping to an equivalent "higher is better" score.
    """
    ...
    # normalize to "higher is better" where best=max_grade
    if (scale_direction or "").lower() == "lower_is_better":
        # If lower is better:  min_passing_grade is typically the worst passing value
        # We invert within [min_passing_grade, max_grade]
        grade_eff = (max_grade + min_passing_grade) - grade
    else:
        grade_eff = grade

    # Now apply formula (higher grade_eff => better => smaller German number)
...
# ============================================================
# FH vs TU heuristic + calibration
# ============================================================
...
def filter_programs(
    ...
    for r in records:
        ...
        # Keep the old keyword mismatch as a SOFT flag only
        if bt and reqtxt:
            strong_terms = [
                "electrical", "computer", "engineering", "business", "informatics",
                "mechanical", "software", "information technology", "it", "physics", "mathematics"
            ]
...
# ============================================================
# HOSTED SEMANTIC RETRIEVAL (DeepSeek embeddings)
# ============================================================
...
def embeddings_cache_get_many(hashes: List[str]) -> Dict[str, List[float]]:
    if not hashes:
        return {}
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    qmarks = ",".join(["?"] * len(hashes))
    cur.execute(f"SELECT text_hash, vector_json FROM embeddings_cache WHERE text_hash IN ({qmarks})", tuple(hashes))
    rows = cur.fetchall()
    conn.close()
...
def embeddings_cache_set_many(items: Dict[str, List[float]]) -> None:
    if not items:
        return
    conn = sqlite3.connect(DB_PATH)
    cur = conn.cursor()
    ...
    for h, vec in items.items():
        cur.execute(
            """
            INSERT OR REPLACE INTO embeddings_cache (text_hash, created_at, model, vector_json)
            VALUES (?, ?, ?, ?)
            """,
            (h, now, DEEPSEEK_EMBED_MODEL, json.dumps(vec)),
        )
    conn.commit()
    conn.close()


def deepseek_embed(texts: List[str], max_retries: int = 2) -> List[List[float]]:
    """
    Calls OpenAI-compatible embeddings endpoint.
    Requires:
      - DEEPSEEK_API_KEY
      - DEEPSEEK_EMBED_URL
      - DEEPSEEK_EMBED_MODEL (must be embeddings-capable)
    """
    if not DEEPSEEK_API_KEY:
        raise RuntimeError("DEEPSEEK_API_KEY missing (required for hosted embeddings).")
    if not DEEPSEEK_EMBED_MODEL:
        raise RuntimeError("DEEPSEEK_EMBED_MODEL is not set. Set it in .env to an embeddings-capable model id.")
    if not DEEPSEEK_EMBED_URL:
        raise RuntimeError("DEEPSEEK_EMBED_URL missing.")

    headers = {"Authorization": f"Bearer {DEEPSEEK_API_KEY}", "Content-Type": "application/json"}
    payload = {"model": DEEPSEEK_EMBED_MODEL, "input": texts}

    last_err = None
    for attempt in range(max_retries + 1):
        try:
            r = requests.post(DEEPSEEK_EMBED_URL, headers=headers, json=payload, timeout=60)
...
def ensure_program_embeddings(texts: List[str]) -> List[List[float]]:
    """
    Returns embeddings aligned with `texts`, using SQLite cache for program texts.
    """
    hashes = [sha_text(t) for t in texts]
    cached = embeddings_cache_get_many(hashes)
...
def retrieve_by_background(
    ...
    # Get embeddings (program embeddings cached; background embedding not cached)
    prog_vecs = ensure_program_embeddings(prog_texts)
    bg_vec = deepseek_embed([bg])[0]

    pairs = []
    for r, v in zip(records, prog_vecs):
        sim = cosine_similarity(bg_vec, v)
        pairs.append((r, sim))
...
# ============================================================
# LLM ranking (DeepSeek chat) + logging
# ============================================================
...
def log_llm_run(
    ...
    temperature: float,
    dataset_hash_ua: str,
    dataset_hash_daad: str,
    student_json: Dict[str, Any],
    packed_programs: List[Dict[str, Any]],
    system_prompt: str,
    user_prompt: str,
    raw_response: str,
...
def deepseek_rank(
    ...
    try:
        ...
        log_llm_run(
            ...
            dataset_hash_ua=dataset_hash_ua,
            dataset_hash_daad=dataset_hash_daad,
            student_json=student_profile,
            packed_programs=packed_programs,
            system_prompt=system_msg,
            user_prompt=json.dumps(user_payload, ensure_ascii=False),
            raw_response=content,
            parsed_json=ranked_items,
            status="ok",
        )
    except Exception as e:
        log_llm_run(
        ...
        # fallback: similarity desc then fewer flags
    ...
    for item in ranked_items:
        ...
        item["background_similarity"] = meta.get("background_similarity")
        ...
        try:
            s = float(item.get("score_0_100", 0.0))
    ...
    enriched.sort(key=lambda x: -(x.get("score_0_100") or 0.0))
...
# ============================================================
# LIFESPAN: load datasets + prepare state
# ============================================================
...
async def lifespan(app: FastAPI):
    ...
    if DEEPSEEK_EMBED_MODEL and DEEPSEEK_API_KEY:
        print(f"✅ DeepSeek embeddings configured | model={DEEPSEEK_EMBED_MODEL}")
        print(f"🌐 Embeddings URL: {DEEPSEEK_EMBED_URL}")
    else:
        print("⚠️ DeepSeek embeddings NOT fully configured – set DEEPSEEK_EMBED_MODEL (and API key)")
...
def recommend(inp: StudentInput):
    ...
    # Convert GPA -> German grade estimate (now supports direction)
    german_est = convert_to_german_grade(
        inp.gpa,
        inp.scale_max,
        inp.min_pass,
        inp.scale_direction,
    ...
    # 1) Semantic retrieval (hosted embeddings) -> candidate set
    # If embeddings not configured, we just fall back to "no semantic signal"
    try:
    ...
    # 2) Hard filters on candidate set
    ...
    # 3) Sort by semantic similarity before LLM (helps pack top N)
    filtered.sort(key=lambda x: float(x.get("_background_similarity") or 0.0), reverse=True)

    # 4) LLM ranking (DeepSeek chat)
    ...
    response = {
        ...
        "notes": {
            "embeddings": {
                "enabled": bool(DEEPSEEK_API_KEY and DEEPSEEK_EMBED_MODEL),
                "embed_url": DEEPSEEK_EMBED_URL,
                "embed_model": DEEPSEEK_EMBED_MODEL or "(not set)",
            }
        },
    }
